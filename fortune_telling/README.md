...
* 逆引き: viewの識別子からviewと対応するURLを得ること
...
ここで、表示したい画面のひな形となるHTMLファイルを用意しておき、おみくじの結果・登録されたユーザ名など、動的に描画したいものだけを埋め込めるようになればうまくいきそうです。
...
※ Django3.1からは、ディレクトリを書くときに参照されるライブラリがosからpathlibへ変更されているので、過去のバージョンのDjangoに触れてきた方は注意が必要です。
...
ですが、DjangoではURLに依存(べた書き)しない別の方法が推奨されています。それは逆引きと呼ばれる、viewへ新たに指定した識別子から対応するURLを得ることです。
...
URLconfへviewの識別子に関する情報が増えました。更に、テンプレートでは、新たにつくられた識別子をよしなに参照しているような記述が追加されています。
一見複雑に見えますが、`Djangoがどのようにviewと対応するURLを得ようとしているのか`理解することを目指して少しずつ見ていきましょう。

### viewの識別子

まずはURLconfから読み解いていきます。app_nameは`application namespaces`とも呼ばれ、viewの識別子を表現する上でのappの名前を設定します。
また、識別子にはURLに利用できる文字に限定するといった制約もないので、例のように日本語を指定することもできます。
...
`{% url 'URL pattern name' %}`のようにviewの識別子を指定すると、対応するviewを呼び出すためのURL文字列が出力として得られます。具体的な例は以下のようになります。
...
これまで見てきた、viewの識別子から対応するURLを得る手法は、なぜ逆引きと名付けられたのでしょうか。少し考えてみましょう。
通常、Djangoでリクエストからレスポンスを得るまでには、「URL→view→HTTPレスポンス」といった順に処理が進んでいきます。
一方、逆引きでは、「viewの識別子→URL」といった順に処理されます。これはURLを始点にHTTPレスポンスを得る上記の処理とは逆に、URLを得ることが終点となっています。
